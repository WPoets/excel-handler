...
\aw2_library::add_service("excel", "Excel Library", [
    "namespace" => __NAMESPACE__,
]);

\aw2_library::add_service(
    "excel.write_bulk",
    "Bulk write in the excel file. Use excel.write_bulk",
    ["namespace" => __NAMESPACE__],
);
function write_bulk($atts, $content = null, $shortcode = null)
{
    if (
    ...
    if ($pageno === 1) {
        ...
        if (array_key_exists("header", $xlsdata)) {
            $objPHPExcel
                ->getActiveSheet()
                ->fromArray($xlsdata["header"], null, "A1");
        }
    } else {
    ...
    if (array_key_exists("rows", $xlsdata)) {
        ...
        $objPHPExcel
            ->getActiveSheet()
            ->fromArray($xlsdata["rows"], null, "A" . $row);
    }
...
\aw2_library::add_service(
    "excel.file_reader",
    "Read the excel file. Use excel.file_reader",
    ["namespace" => __NAMESPACE__],
);
function file_reader($atts, $content = null, $shortcode = null)
{
    if (
        \aw2_library::pre_actions("all", $atts, $content, $shortcode) == false
    ) {
    ...
    $objReader = \PhpOffice\PhpSpreadsheet\IOFactory::createReader(
        $inputFileType,
    );
...
\aw2_library::add_service(
    "excel.info",
    "Read and return the excel info. Use excel.info",
    ["namespace" => __NAMESPACE__],
);
function info($atts, $content = null, $shortcode = null)
{
    if (
        \aw2_library::pre_actions("all", $atts, $content, $shortcode) == false
    ) {
    ...
    extract(
        \aw2_library::shortcode_atts(
            ["file_path" => "", "file_format" => "Excel2007"],
            $atts,
        ),
    );
    if (!is_readable($file_path)) {
...
\aw2_library::add_service(
    "excel.dataset_write",
    "Write the excel file. Use excel.dataset_write",
    ["namespace" => __NAMESPACE__],
);
function dataset_write($atts, $content = null, $shortcode = null)
{
    if (
        \aw2_library::pre_actions("all", $atts, $content, $shortcode) == false
    ) {
    ...
    if ($pageno == 1) {
        ...
        if (array_key_exists("header", $dataset)) {
            $objPHPExcel
                ->getActiveSheet()
                ->fromArray($dataset["header"], null, "A1");
        }
    } else {
        $objPHPExcel = \PhpOffice\PhpSpreadsheet\IOFactory::load($file_path);
        $objPHPExcel->setActiveSheetIndex(0);
    ...
    if (array_key_exists("rows", $dataset)) {
        ...
        $objPHPExcel
            ->getActiveSheet()
            ->fromArray($dataset["rows"], null, "A" . $row);
    }
    $objPHPExcel->getActiveSheet()->getStyle("A1:Z1")->getFont()->setBold(true);
    $objWriter = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter(
        $objPHPExcel,
...
\aw2_library::add_service(
    ...
    "Bulk write the excel file as csv. Use excel.write_bulk_csv",
    ["namespace" => __NAMESPACE__],
);
function write_bulk_csv($atts, $content = null, $shortcode = null)
{
    if (
        \aw2_library::pre_actions("all", $atts, $content, $shortcode) == false
    ) {
...
\aw2_library::add_service(
    "excel.read_header",
    "Read the header info of the excel file. Use excel.read_header",
    ["namespace" => __NAMESPACE__],
);
function read_header($atts, $content = null, $shortcode = null)
{
    if (
        \aw2_library::pre_actions("all", $atts, $content, $shortcode) == false
    ) {
    ...
    extract(
        \aw2_library::shortcode_atts(["filename" => "", "folder" => ""], $atts),
    );

    $file_path = $folder . $filename;
    $objReader = \PhpOffice\PhpSpreadsheet\IOFactory::createReaderForFile(
        $file_path,
    );
...
\aw2_library::add_service(
    ...
    "Read the post data. Use excel.read_post_data",
    ["namespace" => __NAMESPACE__],
);
function read_post_data($atts, $content = null, $shortcode = null)
{
    if (
        \aw2_library::pre_actions("all", $atts, $content, $shortcode) == false
    ) {
